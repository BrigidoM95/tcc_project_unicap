...
class LoginTestCase(TestCase):
    ...
    def test_usuario_com_dados_corretos_deve_conseguir_logar(self):
    ...
    def test_logout_deve_redirecionar_para_login_com_mensagem(self):
        ...
        resposta = self.client.get(reverse('logout'), follow=True)

        self.assertRedirects(resposta, reverse('login'))
        self.assertContains(resposta, 'Você saiu do sistema com segurança.')
    ...
    def test_usuario_logado_deve_acessar_home(self):
        self.client.login(username='admin', password='123456')

        resposta = self.client.get(reverse('home'))

        self.assertEqual(resposta.status_code, 200)
        self.assertContains(resposta, 'Home Administrativa')
